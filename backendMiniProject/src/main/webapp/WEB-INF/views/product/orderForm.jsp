...
<html>
	<head>
		...
		<style>
			...
			table { text-align:left; width: 800px; height:80px}
			th { width:100px; background:#ebdaa6; color:white; text-align:center; }
			td { background:#f1e6c4; padding-left:5px;}
		...
		<div id="wrap">
			...
			<section>
				...
				<form method="post"  action="<c:url value="/product/orderComplete"/>">
					...
					<br>
					<br>
					<h3 id="orderFont">수령인 정보</h3>
					
					<table border="1" width="700">
						<tr>
							<th>받는분</th><td width="200"><input type="text" name="ordReceiver" value="${memDto.memName}" ></td>
							<th>연락처</th>
								<td><input type="text" name="hp1" size="3" value="${hp1}"> - 
										<input type="text" name="hp2" size="4"  value="${hp2}"> - 
										<input type="text" name="hp3" size="4"  value="${hp3}">
								</td>
						</tr>
						<tr>
							<th>배송지 주소</th>
					...
					<table id="cartTable" border="1" width="700">
						<tr><th>상품번호</th><th>사진</th><th>상품명</th><th>가격</th>
				   		...
				   		<c:forEach items="${cartList}" var="ct">   
							 <tr>  
							 	  <td>${ct.prdNo}</td>     					      
							      <td><img src="<c:url value="/image/${ct.cartNo}.png"/>" width="80px" height="80px"></td>					      	
							      <td>${ct.prdName}</td>	      
						...
						<tr>
							...
							<td align="right"><fmt:formatNumber value="${sum}" pattern="#,###" /> 원<br></td>
						</tr>			
					</table><br>
					<br>
					<h3 id="orderFont">결제 정보</h3>
					...
					<br>
					<input type="submit" value="결제완료" class="styled">
				</form>
				</div>
				</section>
